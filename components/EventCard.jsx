  ...
  return (
    <article className="flex flex-col h-full rounded-2xl overflow-hidden bg-white dark:bg-card-dark border border-gray-200 dark:border-card-border group hover:shadow-xl hover:shadow-gray-200/50 dark:hover:shadow-primary/10 transition-all duration-300">
      ...
      <div className="p-5 flex flex-col flex-1 gap-3">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 border-t border-gray-100 dark:border-white/5 pt-3">
        ...
        <div className="flex items-center justify-between mt-1">
          <div className="flex -space-x-2">
            {attendees.map((attendee, index) => (
              <img
                key={index}
                className="w-7 h-7 rounded-full border-2 border-white dark:border-card-dark"
                alt="User Avatar"
                src={attendee}
              />
            ))}
            <div className="w-7 h-7 rounded-full border-2 border-white dark:border-card-dark bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-[10px] font-bold text-gray-600 dark:text-gray-300">
              +{attendeeCount}
            </div>
          </div>
          <button className="bg-primary text-white hover:bg-blue-700 px-4 py-2 rounded-lg text-xs font-bold transition-colors shadow-md shadow-primary/20">
            {buttonText}
          </button>
        </div>
      </div>
